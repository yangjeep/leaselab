...
export async function hashPassword(password) {
    ...
    const keyMaterial = await cryptoAPI.subtle.importKey('raw', encoder.encode(password), { name: 'PBKDF2' }, false, ['deriveBits', 'deriveKey']);
...
export async function hashToken(token, salt) {
    const encoder = new TextEncoder();
    const keyMaterial = await cryptoAPI.subtle.importKey('raw', encoder.encode(token), { name: 'PBKDF2' }, false, ['deriveBits', 'deriveKey']);
    const hash = await cryptoAPI.subtle.deriveBits({
        name: 'PBKDF2',
        salt: salt,
        iterations: 100000,
