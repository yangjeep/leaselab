...
import { getApplicantsByApplicationId, getApplicantById, createApplicant, updateApplicant, deleteApplicant, } from '../lib/db/application-applicants';
import { getDocumentsByApplicationId, getDocumentsByApplicantId, createDocument, updateDocument, verifyDocument, rejectDocument, getDocumentStats, } from '../lib/db/application-documents';
import { getTransitionsByApplicationId, createTransition, } from '../lib/db/application-stage-transitions';
import { getNotesByApplicationId, createNote, updateNote, deleteNote, } from '../lib/db/application-internal-notes';
import { updateLead, getLeadById, recordLeadHistory } from '../lib/db/leads';
const opsApplicationsRoutes = new Hono();
// ==================== APPLICANTS ====================
/**
 * GET /api/ops/applications/:applicationId/applicants
 * Get all applicants for an application
 */
opsApplicationsRoutes.get('/applications/:applicationId/applicants', async (c) => {
    try {
 ...
 */
opsApplicationsRoutes.get('/applicants/:applicantId', async (c) => {
    try {
 ...
 */
opsApplicationsRoutes.post('/applications/:applicationId/applicants', async (c) => {
    try {
        const applicationId = c.req.param('applicationId');
 ...
 * Update an applicant
 */
opsApplicationsRoutes.patch('/applicants/:applicantId', async (c) => {
    try {
        const applicantId = c.req.param('applicantId');
        const body = await c.req.json();
 ...
 * Delete an applicant
 */
opsApplicationsRoutes.delete('/applicants/:applicantId', async (c) => {
    try {
        const applicantId = c.req.param('applicantId');
 ...
 */
opsApplicationsRoutes.get('/applications/:applicationId/documents', async (c) => {
    try {
        const applicationId = c.req.param('applicationId');
 ...
 */
opsApplicationsRoutes.get('/applications/:applicationId/documents/stats', async (c) => {
    try {
        const applicationId = c.req.param('applicationId');
 ...
 */
opsApplicationsRoutes.get('/applicants/:applicantId/documents', async (c) => {
    try {
        const applicantId = c.req.param('applicantId');
 ...
 */
opsApplicationsRoutes.post('/applications/:applicationId/documents', async (c) => {
    try {
        const applicationId = c.req.param('applicationId');
        const userId = c.req.header('X-User-Id');
        const body = await c.req.json();
 ...
 */
opsApplicationsRoutes.patch('/documents/:documentId', async (c) => {
    try {
 ...
 */
opsApplicationsRoutes.post('/documents/:documentId/verify', async (c) => {
    try {
        const documentId = c.req.param('documentId');
        const userId = c.req.header('X-User-Id');
        if (!userId) {
 ...
 * Reject a document
 */
opsApplicationsRoutes.post('/documents/:documentId/reject', async (c) => {
    try {
        const documentId = c.req.param('documentId');
        const userId = c.req.header('X-User-Id');
        const body = await c.req.json();
 ...
 */
opsApplicationsRoutes.get('/applications/:applicationId/transitions', async (c) => {
    try {
        const applicationId = c.req.param('applicationId');
 ...
 */
opsApplicationsRoutes.post('/applications/:applicationId/transitions', async (c) => {
    try {
 ...
 */
opsApplicationsRoutes.get('/applications/:applicationId/notes', async (c) => {
    try {
        const applicationId = c.req.param('applicationId');
 ...
 */
opsApplicationsRoutes.post('/applications/:applicationId/notes', async (c) => {
    try {
        const applicationId = c.req.param('applicationId');
        const userId = c.req.header('X-User-Id');
        const body = await c.req.json();
 ...
 * Update a note
 */
opsApplicationsRoutes.patch('/notes/:noteId', async (c) => {
    try {
 ...
 * Delete a note
 */
opsApplicationsRoutes.delete('/notes/:noteId', async (c) => {
    try {
        const noteId = c.req.param('noteId');
 ...
 */
opsApplicationsRoutes.get('/properties/:propertyId/applications', async (c) => {
    try {
        const siteId = c.req.header('X-Site-Id');
 ...
 */
opsApplicationsRoutes.get('/properties/:propertyId/shortlist', async (c) => {
    try {
        const siteId = c.req.header('X-Site-Id');
        const propertyId = c.req.param('propertyId');
        if (!siteId) {
 ...
 */
opsApplicationsRoutes.post('/applications/:applicationId/shortlist', async (c) => {
    try {
        const siteId = c.req.header('X-Site-Id');
        const applicationId = c.req.param('applicationId');
        const userId = c.req.header('X-User-Id');
        ...
        }); // Type assertion needed until we extend Lead interface
        return c.json({
            success: true,
 ...
 */
opsApplicationsRoutes.delete('/applications/:applicationId/shortlist', async (c) => {
    try {
        const siteId = c.req.header('X-Site-Id');
        const applicationId = c.req.param('applicationId');
        if (!siteId) {
 ...
 */
opsApplicationsRoutes.post('/applications/:applicationId/approve', async (c) => {
    try {
        const siteId = c.req.header('X-Site-Id');
        if (!siteId) {
            return c.json({ error: 'Missing X-Site-Id header' }, 400);
 ...
 */
opsApplicationsRoutes.post('/applications/:applicationId/reject', async (c) => {
    try {
        const siteId = c.req.header('X-Site-Id');
        if (!siteId) {
            return c.json({ error: 'Missing X-Site-Id header' }, 400);
 ...
 */
opsApplicationsRoutes.post('/applications/:applicationId/send-email', async (c) => {
    try {
        const applicationId = c.req.param('applicationId');
        const userId = c.req.header('X-User-Id');
        const body = await c.req.json();
 ...
 */
opsApplicationsRoutes.post('/applications/bulk', async (c) => {
    try {
        ...
        const selectedApps = applications.filter((app) => application_ids.includes(app.id));
        if (selectedApps.length !== application_ids.length) {
            return c.json({ error: 'Some applications not found' }, 404);
        }
        const unitIds = new Set(selectedApps.map((app) => app.unitId).filter(Boolean));
        if (unitIds.size > 1) {
            return c.json({
        ...
        const bulkActionId = await createBulkAction(c.env.DB, userId, action, application_ids.length, params);
        // Execute bulk action
        const results = [];
        let successCount = 0;
        let failureCount = 0;
 ...
 */
opsApplicationsRoutes.post('/applications/:applicationId/proceed-to-lease', async (c) => {
    try {
        const siteId = c.get('siteId');
        const userId = c.get('userId');
        const applicationId = c.req.param('applicationId');
        ...
        const isShortlisted = application.status === 'ai_evaluated' &&
...
function calculateLeaseEndDate(startDate, termMonths) {
