...
import { getLeasesInProgress, updateChecklistStep, completeLeaseOnboarding, } from '../lib/db/lease-onboarding';
 ...
 * }
 */
opsLeasesRoutes.get('/leases/in-progress', async (c) => {
    try {
        const siteId = c.get('siteId');
        if (!siteId) {
 ...
 * }
 */
opsLeasesRoutes.patch('/leases/:id/checklist', async (c) => {
    try {
        const siteId = c.get('siteId');
        const userId = c.get('userId');
        ...
        const updatedChecklist = await updateChecklistStep(c.env.DB, leaseId, step_id, completed, notes);
        // Log audit entry
        await logAuditEntry(c.env.DB, {
            entityType: 'lease',
            entityId: leaseId,
        ...
        return c.json({
            ...
            progress: {
                total_steps: updatedChecklist.total_steps,
                completed_steps: updatedChecklist.completed_steps,
                percentage: Math.round((updatedChecklist.completed_steps / updatedChecklist.total_steps) * 100),
            },
        });
    }
    catch (error) {
 ...
 * }
 */
opsLeasesRoutes.post('/leases/:id/complete-onboarding', async (c) => {
    try {
        const siteId = c.get('siteId');
        const userId = c.get('userId');
        const leaseId = c.req.param('id');
