...
const opsTenantsRoutes = new Hono<{ Bindings: Bindings }>();
 ...
 */
function generateTenantCSV(tenants: any[]): string {
  const headers = [
    'Tenant ID',
    'Full Name',
    'Email',
 ...
 * }
 */
opsTenantsRoutes.post('/tenants/bulk', async (c: Context) => {
  try {
    ...
    const selectedTenants = allTenants.filter((tenant: any) => tenant_ids.includes(tenant.id));
    ...
    for (const tenantId of tenant_ids) {
      try {
        const tenant = selectedTenants.find((t: any) => t.id === tenantId);
