...
export async function getThemeConfiguration(dbInput, siteId) {
    const db = normalizeDb(dbInput);
    const row = await db.queryOne(`SELECT * FROM theme_configurations WHERE site_id = ? LIMIT 1`, [siteId]);
    return mapRow(row);
}
export async function upsertThemeConfiguration(dbInput, siteId, data) {
    const db = normalizeDb(dbInput);
    const now = new Date().toISOString();
    const id = generateId('theme');
    await db.execute(`INSERT INTO theme_configurations (
      id,
      site_id,
      theme_preset,
      brand_name,
