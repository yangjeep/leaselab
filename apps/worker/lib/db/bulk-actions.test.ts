...
describe('bulk-actions database functions', () => {
  ...
  describe('createBulkAction', () => {
    it('should create a bulk action record and return ID', async () => {
      ...
      expect(mockD1.execute).toHaveBeenCalledWith(
        expect.stringContaining('INSERT INTO bulk_actions'),
  ...
  describe('updateBulkActionResults', () => {
    it('should update success and failure counts', async () => {
      await updateBulkActionResults(mockD1 as any, 'bulk_123', 3, 2);

      expect(mockD1.execute).toHaveBeenCalledWith(
        expect.stringContaining('UPDATE bulk_actions'),
        [3, 2, 'bulk_123']
      );
    });
  });
  ...
  describe('getBulkActionById', () => {
    it('should return null if bulk action not found', async () => {
      ...
      const result = await getBulkActionById(mockD1 as any, 'bulk_123');
  ...
  describe('getBulkActionsByUser', () => {
    it('should return empty array when no bulk actions found', async () => {
      ...
      const result = await getBulkActionsByUser(mockD1 as any, 'user_123');
    ...
    it('should return bulk actions for user', async () => {
      ...
      const result = await getBulkActionsByUser(mockD1 as any, 'user_123', 20);
    ...
    it('should use default limit of 20', async () => {
      ...
      await getBulkActionsByUser(mockD1 as any, 'user_123');

      expect(mockD1.query).toHaveBeenCalledWith(
        expect.any(String),
        expect.arrayContaining(['user_123', 20])
      );
    });
  });
  ...
  describe('logAuditEntry', () => {
    it('should create an audit log entry', async () => {
      await logAuditEntry(mockD1 as any, {
